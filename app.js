...
function showStep(index){current=index;steps.forEach((step,i)=>step.classList.toggle('active',i===index));progress.value=index+1;stepLabel.textContent=`Paso ${index+1} de ${steps.length}`;previous.hidden=index===0;next.hidden=index===steps.length-1;submit.hidden=index!==steps.length-1;errorBox.textContent='';steps[index].querySelector('legend')?.focus?.();}
...
form.elements.administracion.forEach(radio=>radio.addEventListener('change',()=>{populateTypes();track('funnel_answer',{step:1,category:radio.value});}));
next.addEventListener('click',()=>{if(!validateStep())return;track('funnel_step_completed',{step:current+1});showStep(current+1)});
...
form.addEventListener('submit',event=>{
  ...
  form.hidden=true;document.querySelector('.assessment-intro').hidden=true;const success=document.querySelector('#success');success.hidden=false;success.focus();track('lead_summary_prepared',{administration:data.get('administracion'),notice_type:data.get('tipo')});
});
